...
colors = ["Blue", "Red", "Green", "black", "purple",
          "pink", "#f89edf", "orange", "Green", "cyan"]
dates = ["180420", "170623", "171113", "180202"]
titles = ["2018-04-20", "2017-06-23", "2017-11-13", "2018-02-02"]
...
for date, title, ax in zip(dates, titles, axs):
    ...
    for station, color in zip(stations, colors):
        ...
        try:
            hour, data = np.loadtxt(
                car+date+"Ery.csv", unpack=True, delimiter=",")
            if(np.mean(data) != 0):
                data = data*40
                ax.plot(hour, data, label=station, c=color,
                        marker=".", ls="none", ms=3, alpha=0.7)
        except:
            pass
    ax.legend(ncol=5, mode="expand", loc="upper center",
              markerscale=4, scatterpoints=1, frameon=False, fontsize=11)
plt.subplots_adjust(left=0.079, bottom=0.09, right=0.955,
                    top=0.921, wspace=0.155, hspace=0.2)
plt.show()
# plt.savefig("../SeasonGraphic/season.png",dpi=400)
