    ...
    if(UVAme[pmeuva] != 0):
        ...
        if (hora >= 12 and hora <= 13):
            ...
            if(int(day[_j]/10000) == 17):
                DRanual[0, 0, 0] += abs(dr)
                DRanual[0, 0, 1] += 1
            else:
                DRanual[0, 1, 0] += abs(dr)
                DRanual[0, 1, 1] += 1
...
for station in stations:
    carp = car+station
    dates = np.loadtxt(car_AOD+"datos340.txt",
    ...
    for date in dates:
        ...
        DRdia = np.zeros([2, 2])
        if ozono[_j] != 0:
            carpmed = carp+vermed+date
            carp_AOD = carp+AOD
            print("Analizando el dia "+file+" de la estacion "+sta[_i])
            UVAme = np.loadtxt(carmed+"UVAme.txt", usecols=1,
                               skiprows=480, max_rows=660)
            Eryme = np.loadtxt(carmed+"Eryme.txt", usecols=1,
                               skiprows=480, max_rows=660)
            UVAmeh = np.loadtxt(carmed+"UVAme.txt", usecols=0,
                                skiprows=480, max_rows=660)
            Erymo = np.loadtxt(carp_AOD+"Eritemica/"+date +
                               "Erymo.txt", usecols=1, skiprows=360)
            UVAmo = np.loadtxt(carp_AOD+"UVA/"+date +
                               "UVAmo.txt", usecols=1, skiprows=360)
            ...
            for _k in range(6*60):
                DRannual, DRdia = promedio(
                    UVAme, UVAmo, DRannual, DRdia, pmeuva, p, _j, DRuva)
                DRannual, DRdia = promedio(
                    Eryme, Erymo, DRannual, DRdia, pmeery, p, _j, DRery)
                hora += 1/60
                pmeuva += 1: pmeery += 1
            if(DRdia[0, 1] != 0):
                DRdia[0, 0] = round(DRdia[0, 0]/DRdia[0, 1], 4)
            if DRdia[1, 1] != 0:
                DRdia[1, 0] = round(DRdia[1, 0]/DRdia[1, 1], 4)
