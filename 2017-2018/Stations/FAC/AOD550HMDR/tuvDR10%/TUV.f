...
! Agregado por Giovanni Gamaliel L¢pez Padilla
       character a
       integer totalarc,arc,i,j,k,n,m,ciclo,si
...
!Se llama al archivo que contiene todos los parametos del dia
          open(100,file="../AOD500nm/datos.txt")
!Se llama al archivo que guardara todos los valores de AOD
          open(101,file="../AOD500nmv2/AODFound.txt")
          read(100,*) totalarc
!Ciclo que sirve para ir variando los dias
       do arc=1,totalarc
       read(100,*) outfil,tauaer,o3_tc,iday,imonth,iyear
       if(o3_tc.ne.0) then
       open(102,file="../Mediciones/v0.0/"//outfil//"UVAme.txt")
       open(103,file="../Mediciones/v0.0/"//outfil//"Eryme.txt")
       open(105,file="../AOD500nmv2/UVA/"//outfil//"UVAmo.txt")
       open(106,file="../AOD500nmv2/Eritemica"//outfil//"Erymo.txt")
       do i=1,599
       read(102,*) hora
       read(103,*) hora
       end do
...
!Ciclo que varia las horas
       ...
       tstart=ciclo
       tstop=ciclo+1
       write(*,*) "de la hora",int(tstart),"a la hora",int(tstop)
        k=0
...
!If que verifica que no se encuentre por debajo el modelo
      if((dif(i).lt.0).and.(sii(i).eq.0)) then
      aod(i)=tauaer
      sii(i)=1
...
!Cierre del do con la condicion
      ...
      write(101,*) outfil,int(tstart),aod(1),aod(2)
!Cierre del ciclo que varia las hora               s
      end do
      hora=10-0.016666
      do i=1,360
      hora=hora+0.016666
