...
def prom(type_c, n_dosis, time_uva, time_uva_mean, car, i, medication, hour_i, hour_f):
    for type_d in range(n_dosis):
        file = open(car+name[i]+"-"+medication[type_d] +
                    "-"+str(type_c)+".csv", "w")
        file.write(",")
        for day in range(365):
            date_name = datetime.date(2019, 1, 1)+datetime.timedelta(days=day)
            date_num = str(date_name.day)
            date_name = date_name.strftime("%b")
        ...
        for hour in range(n_hour):
            ...
            for day in range(365):
                ...
                if time_uva[hour, day, type_d, type_c, 0] != 0:
                    time_uva_mean[hour, month, type_d, type_c,
            ...
            for day in range(365):
                if time_uva[hour, day, type_d, type_c, 0] == 0:
                    month = n_month(day)
                    time_uva[hour, day, type_d, type_c,
                             0] = time_uva_mean[hour, month, type_d, type_c, 0]
                file.write(
                    str(round(time_uva[hour, day, type_d, type_c, 0], 2))+",")
            file.write("\n")
        file.close()
...
for car in carp:
    ...
    for station in stations:
        ...
        for date in dates:
            ...
            data_uva = np.loadtxt(dir_med+date+"UVAmo.txt",
                                  usecols=1, skiprows=hour_i*60, max_rows=n_uv)
            data_uvb = np.loadtxt(dir_med+date+"Erymo.txt",
                                  usecols=1, skiprows=hour_i*60, max_rows=n_uv)
            ...
            for type_c in range(n_cloud):
                for type_d in range(n_dosis):
                    for hour in range(n_hour):
                        time_uva[hour, num, type_d, type_c, 0], time_uva[hour, num, type_d, type_c, 1] = cont(
                            type_d, n_dosis, hour, num, Dosis, n_uv, cloud, time_uva, type_c, data_uva)
...
for type_c in range(n_cloud):
    prom(type_c, n_dosis, time_uva, time_uva_mean,
         car, 0, medication, hour_i, hour_f)
