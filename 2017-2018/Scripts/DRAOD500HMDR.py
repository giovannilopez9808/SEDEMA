...
for station in stations:
    ...
    # <-----------------------Lectura de los dias----------------------------->
    dates = np.loadtxt("../"+station+"/AOD500DM/datos500.txt",
    ...
    for date in dates:
        ...
        DRuva = open(car+"v0.0/DR/"+date+"DRUVA.txt",
                     "w")
        DRery = open(car+"v0.0/DR/"+date+"DREry.txt", "w")
        # <-----------------------Ciclo para los minutos------------------------>
        for k in range(6*60):
            escribir(UVAme, k, DRuva)
            escribir(Eryme, k, DRery)
        ...
        if np.size(DRuva) != 0:
            ...
            name = car + "v0.0/DR/Graficas/"+date+"DRUVA.png"
        ...
        if np.size(DRery) != 0:
            ylabe = ("%Erythemal Relative Difference")
            name = car+"v0.0/DR/Graficas/"+date+"DRery.png"
        ...
        if np.size(DRuva) != 0:
            AODy = AOD[:, 1]
            ylabel = '%RD UVA Irradiance (W/m2)'
            name = car + "v0.0/DR/DRwAOD/"+date+"UVA.png"
            DRwAod(DRuva, AODx, AODy, name, title, ylabel)
        # Verificacion de la existenica de datos
        if np.size(DRery) != 0:
            AODy = AOD[:, 2]
            ylabel = '%RD Erythemal Irradiance (W/m2)'
            name = car + "v0.0/DR/DRwAOD/"+date+"Ery.png"
            DRwAod(DRuva, AODx, AODy, name, title, ylabel)
