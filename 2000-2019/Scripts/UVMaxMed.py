...
for i in range(np.size(lon)):
    ...
    for namestation in station:
        if len(namestation)==3:
            ...
            for file in files:
                year,month,day=int("20"+file[0:2]),int(file[2:4]),int(file[4:6])
                ...
                for hour in range(np.size(data)):
                    if data[hour]>0:
                        ...
                        if annual[year,day,hour]<data[hour]: annual[year,day,hour]=data[hour]
    ...
    for year in range(20):
        ...
        for day in range(365):
            ...
            for hour in range(11,15):
                if max<annual[year,day,hour]: max=annual[year,day,hour]
            #<-----------Calculo del mes------------->
            if max>0:
                month=int(str(datetime.date(2000+year,1,1)+datetime.timedelta(days=day))[5:7])-1
                UVdata[year,day]=max
        ...
        for month in range(12):
            if prom_UV[year,month,1]>0:
                prom_UV[year,month,0]=round(prom_UV[year,month,0]/prom_UV[year,month,1],2)
        ...
        for day in range(365):
            ...
            print(month)
            if UVdata[year,day]>0:
                std[year,month,0]+=(prom_UV[year,month,0]-UVdata[year,day])**2
                std[year,month,1]+=1
        #<--------Ciclo que varia en los meses----------->
        for month in range(12):
            if std[year,month,1]>0:
                std[year,month,0]=np.sqrt(std[year,month,0]/std[year,month,1])
        ...
        for month in range(12):
            if UVmax[year,month,1]!=0:
                UVmax[year,month,0]=round(UVmax[year,month,0]/UVmax[year,month,1],2)    
            if UVmax[year,month,0]!=0:
                #<---------Escritura del archivo---------->
                file.write(str(n)+" "+str(UVmax[year,month,0])+" "+str(std[year,month,0])+"\n")
            n+=1
    file.close()